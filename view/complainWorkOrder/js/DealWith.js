		...
		success: function(obj) {
			...
			if(obj.get("ComplainType") != null) {
				...
				if(ComplainType == 1) {
					...
					$("#results-type").attr('data', 'B');
					$("#ComplainType").html("B类-(需转接其他部门的)");
					var DepName = obj.get("TransferDepartment") ? obj.get("TransferDepartment").get('DepName') : '暂无';
...
function save() {
	subStart();
	var type = $("#results-type").attr('data');
	var result = $("#resultsText").val();
	if(result == '') {
		subEnd();
		layer.msg('处理结果不能为空', {
			shift: 6,
	...
	query.get(ID, {
		success: function(obj) {
			if(type == 'A') {
				obj.set('ResultByService', result);
			} else {
				obj.set('ResultByDepartment', result);
			}
			obj.set('Statu',3);
			obj.save().then(function(obj) {
				layer.confirm('保存流转成功!', {
					btn: ['查看结果', '返回上页'] //按钮
				}, function() {
					window.location.href = "waitingVisit.html";
				}, function() {
					back();
				});
			});
