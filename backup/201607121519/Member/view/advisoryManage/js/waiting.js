...
function main(doctor) {
	...
	query.greaterThanOrEqualTo('Statu', 0);
	query.lessThanOrEqualTo('Statu', 4);
		...
		(function(i) {
			...
			P_query.first({
				success: function(res) {
					...
					count += 1;
					html[i]="<tr>" +'<td>' + MemberName + '</td>' + '<td>' + TypeName + '</td>' + '<td>' + CounselContent + '</td>' + '<td>' + content + '</td>' + '<td>' + chageTime(CreatedTime) + '</td>' + '<td><a href=\"advisoryDetail.html?id=' + Id + '\">详情</a></td>'+'</tr>';
					if (count == len) {
						for (var j=count-1;j>=0;j--) {
							$("#table-body").append(html[j]);
						}
